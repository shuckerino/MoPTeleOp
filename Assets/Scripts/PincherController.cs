// NOTE: adapted from Unity-Technologies/articulations-robot-demo
...
public class PincherController : MonoBehaviour
{
    ...
    void UpdateGrip()
    {
        ...
        float zPositionA = Mathf.Lerp(fingerAController.GetOpenPosition().z, 0, grip);
        float zPositionB = Mathf.Lerp(fingerBController.GetOpenPosition().z, 0, grip);

        fingerAController.transform.localPosition = new Vector3(fingerAController.GetOpenPosition().x, fingerAController.GetOpenPosition().y, zPositionA);
        fingerBController.transform.localPosition = new Vector3(fingerBController.GetOpenPosition().x, fingerBController.GetOpenPosition().y, zPositionB);
    }
